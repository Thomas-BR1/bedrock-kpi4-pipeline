...
def send_daily_email(html_body: str, pull_date: str, email_from: str, email_to: str,
    ...
    Email format:
      - Plain-text summary in the body (Gmail-safe, always readable).
      - Full styled HTML attached as kpi4_daily_<pull_date>.html.
        User clicks the attachment to see the artifact-style rendering.
    """
    import smtplib
    ...
    # Plain-text body — deliberately short. The styled report is the attachment.
    plain = (
        "KPI 4 Daily snapshot for %s.\n\n"
        "The full styled report is attached as an HTML file — open it in your "
        "browser to see the cards, tables, and formatting exactly as they appear "
        "in the Cowork artifact.\n\n"
        "Bedrock Plumbing & Drain Cleaning · Auto-generated by Render.\n"
    ) % pull_date
    msg.attach(MIMEText(plain, "plain"))

    # Attach the styled HTML file
    attachment = MIMEBase("text", "html")
    attachment.set_payload(html_body.encode("utf-8"))
    encoders.encode_base64(attachment)
    filename = "kpi4_daily_%s.html" % pull_date
    ...
    print("Email sent with HTML attachment (%s)." % filename)
